...
from content_owner.api import models
from content_owner.use_cases.auth_service import AuthServiceError
from content_owner.use_cases.commands import ContentOwnerCommands
from content_owner.use_cases.interfaces import AuthUserData
from content_owner.use_cases.queries import ContentOwnerQueries
...
async def create_content_owner(
    ...
    commands: FromDishka[ContentOwnerCommands],
    authorization: Annotated[str | None, Header(alias="Authorization")] = None,
) -> models.ContentOwnerResponse:
    bearer_token = _extract_bearer_token(authorization)
    # try:
    content_owner = await commands.create_content_owner(
...
async def update_content_owner_permissions(
    ...
    try:
        content_owner = await commands.update_permissions(
...
async def delete_content_owner(
    content_owner_id: Annotated[UUID, Path()],
    _: Annotated[
        UserClaims, Depends(authorize(allowed_roles=[Role.ADMIN]))
    ],
    ...
    try:
        content_owner = await commands.delete_content_owner(content_owner_id)
    except DomainError as e:
        raise _map_domain_error(e)
...
async def get_content_owner(
    ...
    queries: FromDishka[ContentOwnerQueries],
) -> models.ContentOwnerResponse:
    try:
        content_owner = await queries.get_content_owner(content_owner_id)
    except DomainError as e:
        raise _map_domain_error(e)
    return models.ContentOwnerResponse.model_validate(
        content_owner, from_attributes=True
