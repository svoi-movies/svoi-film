...
from content_owner.config import AuthClientConfig, Config, JwtConfig
from content_owner.persistence.uow import ContentOwnerUnitOfWork
from content_owner.use_cases import interfaces
from content_owner.use_cases.auth_service import HttpxAuthService
...
class AppProvider(Provider):
    ...
    # Dishka wiring
    uow = provide(ContentOwnerUnitOfWork, provides=interfaces.ContentOwnerUnitOfWork)
    auth_service = provide(HttpxAuthService, provides=interfaces.AuthService)
    commands = provide(ContentOwnerCommands)
    queries = provide(ContentOwnerQueries)

    @provide(scope=Scope.APP)
    def rabbit(self, config: Config) -> RabbitBroker:
        from content_owner.worker.handlers import broker
