...
from company.domain.content_owner import ContentOwner
from company.persistence.schema import content_owners
from company.use_cases.interfaces import ContentOwnerRepository
...
class SqlAlchemyContentOwnerRepository(ContentOwnerRepository):
    ...
    async def get_by_company(
        ...
        result = await self._session.execute(
            sa.select(ContentOwner).where(
                content_owners.c.id == content_owner_id,
                content_owners.c.company_id == company_id,
            )
        )
        content_owner = result.scalar_one_or_none()
        if content_owner is None:
            raise DomainError(
                f"Content owner {content_owner_id} not found for company {company_id}"
            )
        return content_owner
