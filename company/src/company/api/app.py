...
from company.container import create_container
from company.persistence.schema import wire_mappers
...
async def declare_exchanges(container: AsyncContainer) -> None:
    ...
    async with connection.channel() as channel:
        company_exchange = await channel.declare_exchange(
            "company", ExchangeType.TOPIC, durable=True
        )
        company_events = await channel.declare_queue("company.all", durable=True)
        await company_events.bind(company_exchange, routing_key="#")

        content_owner_exchange = await channel.declare_exchange(
            "content-owner", ExchangeType.TOPIC, durable=True
        )
        content_owner_events = await channel.declare_queue(
            "content-owner.all", durable=True
        )
        await content_owner_events.bind(content_owner_exchange, routing_key="#")
