...
from subscriptions.persistence.schema import wire_mappers
...
async def declare_exchanges(container: AsyncContainer) -> None:
    ...
    async with connection.channel() as channel:
        subscriptions_exchange = await channel.declare_exchange(
            "subscriptions", ExchangeType.TOPIC, durable=True
        )
        all_subscriptions = await channel.declare_queue(
            "subscriptions.all", durable=True
        )
        await all_subscriptions.bind(subscriptions_exchange, routing_key="#")
