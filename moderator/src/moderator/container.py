...
from moderator.config import AuthClientConfig, Config, JwtConfig
from moderator.persistence.uow import ModerationUnitOfWork
...
class AppProvider(Provider):
    ...
    # Dishka wiring
    uow = provide(ModerationUnitOfWork, provides=interfaces.ModerationUnitOfWork)
    auth_service = provide(HttpxAuthService, provides=interfaces.AuthService)
    commands = provide(ModerationCommands)
    queries = provide(ModerationQueries)

    @provide(scope=Scope.APP)
    def rabbit(self, config: Config) -> RabbitBroker:
        from moderator.worker.handlers import broker
