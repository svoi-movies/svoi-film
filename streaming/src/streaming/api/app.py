...
async def declare_exchanges(container: AsyncContainer) -> None:
    ...
    async with connection.channel() as channel:
        streaming_exchange = await channel.declare_exchange(
            "streaming", ExchangeType.TOPIC, durable=True
        )
        all_streaming = await channel.declare_queue("streaming.all", durable=True)
        await all_streaming.bind(streaming_exchange, routing_key="#")
